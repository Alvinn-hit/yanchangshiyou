...
function getDropDownList(data, tagId, headerValue, attrName, defaultValue){
	var tag = $api.dom("#" + tagId);
	...
	for(var i = 0; i < arr.length; i++){
		if(arr[i][attrName] == defaultValue){
			html += htmlTemplet.replace("-99", arr[i][attrName]).replace("全部" , arr[i][attrName]).replace("{flag}" , "selected='selected'");
		}else{
			html += htmlTemplet.replace("-99", arr[i][attrName]).replace("全部", arr[i][attrName]).replace("{flag}" , "");;
		}
	}
	$api.html(tag, html);
	$api.attr(tag, "style", "display:;"); 				
...
function getWorkBookHtmlByList(data, tagId, flag){
	...
	if(arr.length > 0 || flag){
		...
		html = '<div class="aui-content"><div class="no-img-box"><img src="../../image/nothing.png" />暂无信息</div></div>';
